...
/*   Updated: 2019/11/11 16:06:33 by sdunckel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	bitmap_file_header(t_mini_rt *rt, int padding_size, int fd)
{
	...
	file_size = BMP_HEADER_SIZE + (rt->img.bpp / 8 * rt->res.x + padding_size)
				* rt->res.y;
...
void	bitmap_info_header(t_mini_rt *rt, int fd)
{
	...
	info_header[14] = (unsigned char)(rt->img.bpp);
...
void	create_bmp_image(t_mini_rt *rt, char *file_name)
{
	...
	while (i >= 0)
	{
		write(fd, rt->img.add + (i * rt->res.x * rt->img.bpp / 8),
				rt->img.size_line);
